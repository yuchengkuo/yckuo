...
<div
  class="rounded-full bg-surface flex items-center divide-x divide-border"
  children="py-2 hover:bg-fg/8 active:bg-fg/6 rounded-inherit"
>
  <button class="pl-5 pr-3 rounded-r-none text-base font-Azeret tracking-tighter" on:click={upvote}>
    {#if up}
      ...
      >
        ...
        <path
          ...
          fill="current"
          use:motion={{
            animate: { fill: ['pink', 'yellow', 'lightgreen', 'currentColor'] },
  ...
  <button class="pl-3 pr-5 rounded-l-none" on:click={downvote}>
    {#if down}
