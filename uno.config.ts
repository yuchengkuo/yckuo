...
const radixThemes = Object.fromEntries(
  Object.entries(Radix).filter(([k]) => !k.match(/Dark/))
)
const radixThemesDark = Object.fromEntries(
  Object.entries(Radix).filter(([k]) => k.match(/Dark/))
)
  ...
  shortcuts: [
    {
      ...
      button:
        'bg-transparent hover:(bg-fg-muted text-bg-muted) duration-100 active:bg-fg',
    ...
    [
      /^border-rx-(.*)$/,
      ([, r]) => `border-radix-${r} dark:border-radix-dark-${r}`,
    ],
  ],
  variants: [
    {
...
function removeRadixColorPrefix(radix = {}) {
  return Object.fromEntries(
    Object.entries(radix).map(([k, v]) => [k.match(/\d+/), v])
  )
}
