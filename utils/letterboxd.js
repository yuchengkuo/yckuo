...
function getPublishedDate(element) {
  return element.find('pubDate').text()
}

function getWatchedDate(element) {
  return element.find('letterboxd\\:watchedDate').text()
}
  ...
  $('p').each((i, paragraphElement) => {
    ...
    if (isPlusMoreParagraph) {
      ...
      const startNumberPosition =
        text.indexOf(startNumberPositionString) + startNumberPositionString.length
...
export const getLetterboxd = async () => {
  return await letterboxd('ethank')
}
